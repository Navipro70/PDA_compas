...
public class Engine implements Jsonable {
    ...
    private Engine() {
        ...
        startCommands = this.commands.filter((cmd) -> Objects.equals(cmd, START));
        startCommands
    ...
    private void startGame(Game game) {
        ...
        CodeInputEventBus.getCodeScans().observeOn(computation).subscribe((code) -> {
            boolean accepted = game.acceptCode(code);
            CodeInputEventBus.codeAccepted(accepted);
            if (accepted) {
                // Если код был принят, отправляем уведомление с самим кодом
                CodeInputEventBus.notifyCodeAccepted(code);
            }
        });
        ...
        ItemEventsBus.instance().getItemUsedEvents()
                .filter((i) -> Objects.equals(i.getItemDescriptor().getName(), "Anabiotic"))
    ...
    private Disposable setupStateTimers() {
        ...
        cd.add(makeCountDownForStates(Player.STATE.W_DEAD_ANOMALY, Player.STATE.DEAD_ANOMALY));
        cd.add(makeCountDownForStates(Player.STATE.W_DEAD_FRUITPUNCH, Player.STATE.DEAD_BURER));
        cd.add(makeCountDownForStates(Player.STATE.W_DEAD_EXPLOSION, Player.STATE.DEAD_EXPLOSION));

        // Добавляем таймеры для состояний смерти
        cd.add(makeCountDownForStates(Player.STATE.DEAD_BURER, Player.STATE.ALIVE));
        cd.add(makeCountDownForStates(Player.STATE.DEAD_ANOMALY, Player.STATE.ALIVE));
        cd.add(makeCountDownForStates(Player.STATE.DEAD_RADIATION, Player.STATE.ALIVE));
        cd.add(makeCountDownForStates(Player.STATE.DEAD_CONTROLLER, Player.STATE.ALIVE));
        cd.add(makeCountDownForStates(Player.STATE.DEAD_MENTAL, Player.STATE.ALIVE));
        cd.add(makeCountDownForStates(Player.STATE.DEAD_EXPLOSION, Player.STATE.ALIVE));
        return cd;
    }
...
//            Log.e(TAG, calToStr(Calendar.getInstance()));
            ...
            if (now < (strtAt.getTimeInMillis() - (long) e.notifyBefore() * 1000 * 60)) {
                executor.postAction(new Action() {
                    ...
                    public long startTime() {
                        ...
                        return strtAt.getTimeInMillis() - (long) e.notifyBefore() * 1000 * 60;
                    }
    ...
    private Disposable setupSuicides() {
        Observable<String> suicideRequests = PlayerEventBus.instance().getPlayerCommandsStream().filter((c) -> Objects.equals(c, PlayerEventBus.SUICIDE));
    ...
    private class PlayerEventsListenerImpl implements PlayerEventsListener {
        ...
        public void onPlayerStateChanged(Player.STATE oldState, Player.STATE newState) {
            ...
            if (newState == Player.STATE.W_DEAD_ANOMALY) {
                countdownStarted.onNext(System.currentTimeMillis());
                net.afterday.compas.logging.Logger.e(R.string.message_suicide);
            ...
            if (newState == Player.STATE.W_DEAD_FRUITPUNCH) {
                countdownStarted.onNext(System.currentTimeMillis());
                net.afterday.compas.logging.Logger.d(R.string.message_suicide);
                currentPlayerState.onNext(Player.STATE.W_DEAD_FRUITPUNCH);
                controls.stopInfluences();
                return;
            }
            if (newState == Player.STATE.W_DEAD_EXPLOSION) {
                countdownStarted.onNext(System.currentTimeMillis());
                net.afterday.compas.logging.Logger.e(R.string.message_w_dead_explosion);
                currentPlayerState.onNext(Player.STATE.W_DEAD_EXPLOSION);
                controls.stopInfluences();
                return;
            }
            if (newState == Player.STATE.DEAD_EXPLOSION) {
                net.afterday.compas.logging.Logger.e(R.string.message_dead_explosion);
                currentPlayerState.onNext(Player.STATE.DEAD_EXPLOSION);
                controls.startInfluences();
                return;
            }
        ...
        public void onFractionChanged(Player.FRACTION newFraction, Player.FRACTION oldFraction) {
            switch (newFraction) {
                case STALKER:
                    Logger.d(R.string.message_fraction_stalker);
                    break;
                case MONOLITH:
                    Logger.d(R.string.message_fraction_monolith);
                    break;
                case GAMEMASTER:
                    Logger.d(R.string.message_fraction_gamemaster);
                    break;
                case DARKEN:
                    Logger.d(R.string.message_fraction_darken);
                    break;
                case MISSION:
                    Logger.d(R.string.message_fraction_mission);
                    break;
                default:
                    Logger.d("Fraction changed to " + newFraction);
                    break;
            }
